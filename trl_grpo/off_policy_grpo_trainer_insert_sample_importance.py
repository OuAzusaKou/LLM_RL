...
class GRPOTrainer(Trainer):
    ...
    def _prepare_inputs(self, inputs: dict[str, Union[torch.Tensor, Any]]) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        if self.args.use_vllm:
            ...
            # 检查是否有预定答案需要替换
            if self.accelerator.is_main_process:
                if 'extra_info' in inputs[0] and 'answer' in inputs[0]['extra_info']:
                    target_prompt = prompts_text[0]
                    answer = inputs[0]['extra_info']['answer']
                    
                    # 找到与目标prompt相同的样本索引
                    matching_indices = [i for i, p in enumerate(prompts_text) if p == target_prompt]
                    
                    if matching_indices:
                        # 随机选择一个匹配的索引进行替换
                        replace_idx = random.choice(matching_indices)
                        # 将答案转换为token ids
                        ...
                        completion_ids[replace_idx] = answer_ids
                    else:
                        print(f"警告：未找到与目标prompt匹配的样本，无法替换预定答案。")
        ...
        current_logps = gather_object(ref_per_token_logps)
        ...
        #计算baseline (加权概率平均)
        mean_grouped_rewards = (importance_weights * rewards).sum() / (importance_weights.sum() + 1e-8)
