...
class GRPOTrainer(Trainer):
    ...
    def advantage_compute(self, model, inputs):
        ...
        for key, samples in prompt_samples.items():
            ...
            # Padding
            padded_prompt_ids = pad(prompt_ids_list, padding_value=self.processing_class.pad_token_id, padding_side="left")
            padded_prompt_mask = pad(prompt_mask_list, padding_value=0, padding_side="left")
            padded_completion_ids = pad(completion_ids_list, padding_value=self.processing_class.pad_token_id, padding_side="right")
            padded_completion_mask = pad(completion_mask_list, padding_value=0, padding_side="right")
            padded_ref_logps = pad(ref_logps_list, padding_value=0, padding_side="right")
            ...
            # 计算当前策略下的logps
            with torch.inference_mode():
                current_logps = self._get_per_token_logps(model, input_ids, attention_mask, max_completion_len)
