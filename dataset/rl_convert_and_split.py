...
def convert_format_pure_code(input_file, output_file):
    """转换单个文件的格式"""
    ...
    if len(data['instruction']) > 0:
        ...
        return None
    question = str(f"任务：这段代码可能存在一个问题（质量缺陷或者安全漏洞），以下是缺陷相关代码，请据此判断缺陷/漏洞是否存在。\n\n代码：\n{input_data['code']}\n\n 输出格式：\n根据以上详细信息，请做出缺陷是否存在的判断，如果缺陷真实存在，请在回复的开头给出'缺陷真实存在'的结论，然后给出判断的理由，如过缺陷不存在，请在回复的开头给出'缺陷不存在'的结论，然后给出判断的理由；\n以下是两个回复的示例，一个是缺陷不存在的，一个是缺陷真实存在的，供参考。\n 回复示例1：\n\n## 缺陷判断\n\n 缺陷真实存在\n\n \n\n ## 缺陷真实存在的原因分析\n\n 1. xxx\n\n 2. xxx\n\n \n\n ####相关事件:事件1. 行号: 668, 函数 __ioat_prep_pq_lock 使用参数 src_cnt 来访问内存缓冲区，这可能会导致崩溃或计算错误。内存区间大小： 8 字节，偏移： 255 字节。 \n 回复示例2：\n\n \n\n ## 缺陷真实性判断\n\n 缺陷不存在。 \n 需要返回与代码相关的\"判断\",和\"相关事件\"，请一步一步思考，并用 \"##\" 来提示判断结果, 用\"####\"来提示相关事件。\n")
    answer = str(f"判断:{'缺陷真实存在' if output_data['judgement']=='缺陷真实存在' else '缺陷不存在'}\n\n原因：{output_data['reason']}")
...
def convert_format_sft(input_file, output_file):
    """转换单个文件的格式"""
    with open(input_file, 'r', encoding='utf-8') as f:
        data = json.load(f)
    ...
    if len(data['instruction']) > 0:
        ...
        return None
    answer = str(f"判断:{output_data['judgement']}\n\n原因：{output_data['reason']}")
    new_format = {
        "subtype": subtype,
        "prompt": [
            {
                "role": "user",
                "content": str(f"任务：{input_data['task']}\n\n代码：\n{input_data['code']}\n\n事件：\n{input_data['event']}\n\n输出格式：\n{input_data['output_format']},请先一步一步思考，并最后用 \"##\" 来提示判断结果")
            }
        ],
        'reward_model': {
                'explanation': input_data['event'],
    ...
    for root, dirs, files in os.walk(input_dir):
        for filename in files:
            if filename.endswith('.json'):
                ...
                subtype = convert_format_pure_code(input_file, output_file)
...
def main():
    input_dir = "./dataset/output"  # 输入目录
    converted_file = "./dataset/rl_converted_data.jsonl"
    train_file = "./dataset/rl_train.jsonl"
    test_file = "./dataset/rl_test.jsonl"
    test_ratio = 0.2
